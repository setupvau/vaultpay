...
const STEPS = {
  SELECT:   'select',
  FORM:     'form',
  WAITING:  'waiting',   // INR: waiting for admin to assign bank
  ORDER:    'order',     // bank assigned, show details
  PROOF:    'proof',     // user submits payment proof
  DONE:     'done',
};
...
export default function DepositPage() {
  ...
  // Poll backend every 8 seconds while waiting for bank assignment
  ...
  // Countdown timer once bank is assigned
  useEffect(() => {
  ...
  const submitUSDT = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await depositAPI.createUSDT({ amount_usdt: parseFloat(usdtAmount), chain_type: chain });
      setOrder(data.data);
      setStep(STEPS.ORDER);
    } catch (err) {
    ...
    <Layout>
      <div className="max-w-lg mx-auto px-4 py-5">
        <div className="flex items-center gap-3 mb-6 pt-1">
          <Link to="/dashboard" className="w-9 h-9 rounded-xl bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-white">←</Link>
          <div>
        ...
        <div className="space-y-3">
          {[
            { t:'inr',  icon:'🏦', title:'Deposit INR',  desc:'Bank transfer / UPI  ·  Earn daily profit', color:'border-emerald-800/40 hover:border-emerald-600' },
            { t:'usdt', icon:'₮',  title:'Deposit USDT', desc:'TRC20 or BEP20  ·  Credited as INR  ·  No lock period',  color:'border-yellow-800/40 hover:border-yellow-600'  },
          ].map(({ t, icon, title, desc, color }) => (
            <button key={t} onClick={() => { setType(t); setStep(STEPS.FORM); }}
    ...
    <Layout>
      <div className="max-w-lg mx-auto px-4 py-5">
        <div className="flex items-center gap-3 mb-5 pt-1">
          <button onClick={() => setStep(STEPS.SELECT)} className="w-9 h-9 rounded-xl bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-white">←</button>
          <div><h1 className="text-xl font-black text-white">Deposit USDT</h1><p className="text-gray-500 text-xs">Credited as INR at current rate</p></div>
        </div>
          ...
          <div className="card rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs bg-yellow-900/40 text-yellow-400 border border-yellow-800/40 px-2.5 py-1 rounded-full font-semibold">
                Rate: 1 USDT = ₹{/* rate shown from order */}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {['TRC20','BEP20'].map(c => (
                <button key={c} type="button" onClick={() => setChain(c)}
                  className={`rounded-xl border py-3.5 font-bold text-sm transition-all ${chain===c ? 'border-yellow-500 bg-yellow-900/20 text-yellow-300' : 'border-gray-700 text-gray-400 hover:border-gray-600'}`}>
                  {c}
                  <p className="text-xs font-normal text-gray-500 mt-0.5">{c==='TRC20' ? 'Lower fees' : 'BNB Chain'}</p>
                </button>
              ))}
            </div>
          </div>
  ...
  if (step === STEPS.FORM && type === 'inr') return (
    <Layout>
      <div className="max-w-lg mx-auto px-4 py-5">
        <div className="flex items-center gap-3 mb-5 pt-1">
          <button onClick={() => setStep(STEPS.SELECT)} className="w-9 h-9 rounded-xl bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-white">←</button>
          <div><h1 className="text-xl font-black text-white">Deposit INR</h1><p className="text-gray-500 text-xs">Bank transfer or UPI</p></div>
        </div>
        ...
        <div className="rounded-2xl bg-blue-900/20 border border-blue-800/30 p-4 mb-4">
          ...
          <div className="space-y-1.5">
            {['Enter your deposit amount','We assign you a bank account (1-5 min)','Transfer the exact amount','Upload your payment screenshot','Admin confirms and credits your wallet'].map((s,i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="w-5 h-5 rounded-full bg-blue-800 text-blue-300 text-xs flex items-center justify-center shrink-0 mt-0.5 font-bold">{i+1}</span>
        ...
        <form onSubmit={submitINR} className="space-y-4">
          <div className="card rounded-2xl p-5">
            ...
            <p className="text-xs text-gray-600 mt-2">Earn daily profit on deposits</p>
          </div>
  ...
  // ════════════════════════════════════════
  // RENDER: WAITING FOR BANK ASSIGNMENT
  // ════════════════════════════════════════
  ...
  // ════════════════════════════════════════
  // RENDER: ORDER DETAILS (bank assigned)
  // ════════════════════════════════════════
  if (step === STEPS.ORDER) return (
    <Layout>
      <div className="max-w-lg mx-auto px-4 py-5">
        <div className="flex items-center gap-3 mb-5 pt-1">
          <div><h1 className="text-xl font-black text-white">{type==='usdt' ? 'Send USDT' : 'Transfer Money'}</h1>
          <p className="text-gray-500 text-xs">Complete your payment then upload proof</p></div>
        </div>

        {/* Countdown for INR orders */}
        {type === 'inr' && order?.expires_at && timeLeft !== null && (
          <div className={`rounded-2xl p-4 mb-4 text-center border ${timeLeft < 300000 ? 'bg-red-900/20 border-red-800/40' : 'bg-amber-900/20 border-amber-800/30'}`}>
        ...
        <div className="card rounded-2xl overflow-hidden mb-4">
          <div className="bg-gradient-to-r from-gray-800 to-gray-900 px-5 py-4 border-b border-gray-700">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-emerald-400 bg-emerald-900/30 px-2 py-0.5 rounded-full border border-emerald-800/40">✓ Bank Assigned</span>
            </div>
          ...
          <div className="p-5 space-y-3 text-sm">
            {type === 'usdt' && <>
              ...
              <InfoRow label="INR Value" value={`₹${parseFloat(order?.inr_equivalent||0).toLocaleString('en-IN')}`} />
            </>}
            {type === 'inr' && order?.bank_details && <>
              <InfoRow label="Bank"           value={order.bank_details.bank_name || order.bank_details.account_holder} />
              <InfoRow label="Account Holder" value={order.bank_details.account_holder} />
              {order.bank_details.account_number && <CopyRow label="Account No." value={order.bank_details.account_number} />}
              {order.bank_details.ifsc_code      && <CopyRow label="IFSC Code"   value={order.bank_details.ifsc_code} />}
              {order.bank_details.upi_id         && <CopyRow label="UPI ID"      value={order.bank_details.upi_id} />}
            </>}
  ...
  if (step === STEPS.PROOF) return (
    <Layout>
      <div className="max-w-lg mx-auto px-4 py-5">
        <div className="flex items-center gap-3 mb-5 pt-1">
          <button onClick={() => setStep(STEPS.ORDER)} className="w-9 h-9 rounded-xl bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-white">←</button>
          <div><h1 className="text-xl font-black text-white">Upload Proof</h1><p className="text-gray-500 text-xs">Screenshot + reference number</p></div>
        </div>
        ...
        <form onSubmit={submitProof} className="space-y-4">
          <div className="card rounded-2xl p-5">
            ...
            <input type="text" className="input" required
              placeholder={type==='usdt' ? 'Paste transaction hash from your wallet' : 'Enter bank UTR or UPI reference number'}
          ...
          <div className="card rounded-2xl p-5">
            ...
            {screenshotPreview ? (
              <div className="relative rounded-xl overflow-hidden">
                <img src={screenshotPreview} alt="Preview" className="w-full max-h-64 object-contain bg-gray-800" />
                <button type="button"
          ...
          <button type="submit" disabled={loading || !screenshot} className="btn-primary w-full py-4 text-base disabled:opacity-40">
  ...
  if (step === STEPS.DONE) return (
    <Layout>
      <div className="max-w-lg mx-auto px-4 py-10 text-center">
        ...
        <p className="text-gray-500 text-sm mb-8">Usually confirmed within 1 hour. You'll get a notification.</p>
...
function InfoRow({ label, value, mono }) {
  return (
    <div className="flex items-center justify-between gap-2">
      <span className="text-gray-500 shrink-0 text-sm">{label}</span>
      <span className={`text-gray-200 font-semibold text-sm text-right ${mono ? 'font-mono text-xs' : ''}`}>{value}</span>
    </div>
  );
}
...
function R({ label, value, mono }) {
  return (
    <div className="flex justify-between gap-2 py-1.5 border-b border-gray-800 last:border-0">
      <span className="text-gray-500 shrink-0 text-sm">{label}</span>
      <span className={`text-gray-200 font-semibold text-sm text-right break-all ${mono ? 'font-mono text-xs' : ''}`}>{value}</span>
    </div>
  );
}
